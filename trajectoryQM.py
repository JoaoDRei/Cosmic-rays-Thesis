...
class Particle:
    ...
    def larmor_radius(self,B):
        ...
        return v_perp / (abs(self.qoverm) * B_mag)
    def gyrofrequency(self, B):
        B_mag= np.linalg.norm(B)
        return abs(self.qoverm) * B_mag
# ----------------------------
# Field evaluators
# ----------------------------
def EvalE(pos):
...
FIELD_MODELS = {
    ...
    "TwistTanh": TwistTanh,
    "TwistTanhPar": TwistTanhPar,
}
B_MODEL_NAME = "TwistTanh" # Choose the magnetic field model here
...
def UpdateVelocityForward(part, E, B, dt):
    ...
    for i in range(3):
        ...
        part.v[i] += (part.qoverm) * (E[i] + vxB[i]) * dt
def UpdateVelocityBoris(part, E, B, dt):
    ...
    t = [(part.qoverm) * B[i] * 0.5 * dt for i in range(3)] # t is the normalized magnetic field vector scaled by the time step and charge-to-mass ratio t=(q/m)*B*dt/2
    ...
    # v-
    for i in range(3):
        v_minus[i] = part.v[i] + (part.qoverm) * E[i] * 0.5 * dt #defining v- as the velocity after the half electric field acceleration, calculated as v- = v + (q/m)*E*dt/2. In this case, the electric field is zero, so v- is equal to the initial velocity v.
        #v_minus[i] = part.v[i] + (part.q/part.m) * E[i] * 0.5 * dt
    # v'
    ...
    # final velocity
    for i in range(3):
        part.v[i] = v_plus[i] + (part.qoverm) * E[i] * 0.5 * dt
...
def SampleParticleLr():
    ...
    part.v[1] = 1 # sets the y-component of the velocity to 100,000 m/s
...
def SampleParticle2():
    part = Particle() #initiates a particle with zero position and velocity, and the charge and mass of an electron
    part.x[1]=1
    part.v[1] = 1 # sets the y-component of the velocity to 0.1 m/s
    part.v[2] = 1 # sets the z-component of the velocity to 10,000 m/s, which means that the particle has a small velocity component along the magnetic field direction (z-axis) in addition to its larger velocity component perpendicular to the magnetic field (y-axis). This setup allows us to observe both the gyration around the magnetic field lines and the motion along the field lines.
    B = EvalB(part.x)  # evaluates the magnetic field at the particle's position
...
def main():
    ...
    with open(outdir / "trace.txt", "w") as f: # opens a file named "trace.txt" in write mode. This file will be used to store the output of the simulation, including the time step, time, position, and velocity of the particle at each recorded step.
        ...
        part = SampleParticleLr()  #SampleParticleLr(), SampleParticle2() 
        ...
        B = EvalB(part.x)
        B_mag = np.linalg.norm(B)
        dt= 0.01*2*np.pi/(part.qoverm *B_mag)  # dt defined as 1/100 of the initial gyroperiod (which may change since B may also change)
